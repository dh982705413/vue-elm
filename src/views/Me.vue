<template>
  <div class="me">
    ...
    <div v-if="userInfo">
      ...
      <button class="loginOut-btn" @click="handleLogout">
        退出登录
      </button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      // eslint-disable-next-line camelcase
      const res = await this.$http.get(`/user/user_info/${user_id}`)
      res.phone = res.phone.split('')
      res.phone.splice(4, 4, '****')
      res.phone = res.phone.join('')
      this.userInfo = res
    }
  }
}
