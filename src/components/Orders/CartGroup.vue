<template>
  <section class="checkout-section cart-group" v-if="orderInfo">
    ...
    <ul>
      <li v-for="(food, index) in orderInfo.selectFoods" :key="index">
        ...
        <div class="cart-group-info">
          ...
          <span>￥{{ food.activity.fixed_price * food.count }}</span>
        </div>
      </li>
      ...
      <li class="cart-group-total">
        ...
        <div class="subtotal">
          ...
          <span class="price"
            >￥{{
              (totalPrice + orderInfo.shopInfo.float_delivery_fee).toFixed(1)
            }}</span
          >
        </div>
      </li>
